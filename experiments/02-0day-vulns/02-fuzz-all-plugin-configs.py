...
def main():
    ...
    for leaf_dir in leaf_dirs:
        ...
        for fuzzed_function in fuzzed_functions:
            ...
            the_containers = ['web'] + fuzzer_names
            # Kill all container
            ...
            run_docker_command(f"up -d --force-recreate {' '.join(fuzzer_names)}", os.path.abspath(DIRS['code']), docker_compose_name)
            time.sleep(FUZZ_TIME) # Run fuzzers for 5s
            run_docker_command(f"kill {' '.join(the_containers)}", os.path.abspath(DIRS['code']), docker_compose_name)
